package net.dark_roleplay.library.unstable.experimental.guis;
...
import net.dark_roleplay.library.unstable.experimental.guis.modular.ModularGui_Template;
import net.dark_roleplay.library.unstable.experimental.guis.modular.Modular_Background;
import net.dark_roleplay.library.unstable.experimental.guis.modular.Modular_Buttons;
...
public class ModularGui_Handler implements ISelectiveResourceReloadListener {

	
	public static class GuiDesign implements IResourceType{}

	@Override
	public void onResourceManagerReload(IResourceManager resourceManager, Predicate<IResourceType> resourcePredicate) {
		if(!resourcePredicate.test(new GuiDesign())) return;
		DRPLibrary.LOGGER.info("+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=");
		DRPLibrary.LOGGER.info("Starting to load Dark Roleplay modular guis");
		DRPLibrary.LOGGER.info("+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=");
		
//		ClientProxy.modularGuis.add();
		ModularGuiHandler.currentGui = loadModularGui(new ResourceLocation(DRPLibrary.MODID, "textures/guis/modular_gui/vanilla.json"));//ClientProxy.modularGuis.get(0);

		DRPLibrary.LOGGER.info("+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=");
		DRPLibrary.LOGGER.info("Finished loading of Dark Roleplay modular guis");
		DRPLibrary.LOGGER.info("+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=");
	}
		...
		try {
			...
			if(version_json != null && name_json != null){
				...
				if(background_json != null){
					...
					if(path == null){
						DRPLibrary.LOGGER.warn("Modular Backgrounds: Cannot find 'path' for 'background'");
						skip = true;
					}else if(path_hollow == null){
						DRPLibrary.LOGGER.warn("Modular Backgrounds: Cannot find 'path_hollow' for 'background'");
						skip = true;
					}else if(left_json == null){
						DRPLibrary.LOGGER.warn("Modular Backgrounds: Cannot find 'left' for 'background'");
						skip = true;
					}else if(right_json == null){
						DRPLibrary.LOGGER.warn("Modular Backgrounds: Cannot find 'right' for 'background'");
						skip = true;
					}else if(top_json == null){
						DRPLibrary.LOGGER.warn("Modular Backgrounds: Cannot find 'top' for 'background'");
						skip = true;
					}else if(bottom_json == null){
						DRPLibrary.LOGGER.warn("Modular Backgrounds: Cannot find 'bottom' for 'background'");
						skip = true;
					}else if(width_json == null){
						DRPLibrary.LOGGER.warn("Modular Backgrounds: Cannot find 'width' for 'background'");
						skip = true;
					}else if(height_json == null){
						DRPLibrary.LOGGER.warn("Modular Backgrounds: Cannot find 'height' for 'background'");
						skip = true;
					}
				...
				if(buttons_json != null){
					...
					if(pathDisabled == null){
						DRPLibrary.LOGGER.warn("Modular Backgrounds: Cannot find 'path_disabled' for 'buttons'");
						skip = true;
					}else if(pathEnabled == null){
						DRPLibrary.LOGGER.warn("Modular Backgrounds: Cannot find 'path_enabled' for 'buttons'");
						skip = true;
					}else if(pathHovered == null){
						DRPLibrary.LOGGER.warn("Modular Backgrounds: Cannot find 'path_hovered' for 'buttons'");
						skip = true;
					}else if(left_json == null){
						DRPLibrary.LOGGER.warn("Modular Backgrounds: Cannot find 'left' for 'buttons'");
						skip = true;
					}else if(right_json == null){
						DRPLibrary.LOGGER.warn("Modular Backgrounds: Cannot find 'right' for 'buttons'");
						skip = true;
					}else if(top_json == null){
						DRPLibrary.LOGGER.warn("Modular Backgrounds: Cannot find 'top' for 'buttons'");
						skip = true;
					}else if(bottom_json == null){
						DRPLibrary.LOGGER.warn("Modular Backgrounds: Cannot find 'bottom' for 'buttons'");
						skip = true;
					}else if(width_json == null){
						DRPLibrary.LOGGER.warn("Modular Backgrounds: Cannot find 'width' for 'background'");
						skip = true;

...
public final class IntArrayCompression {
	...
	public static int[] compress(int bitCount, int[] data) {
		...
		for(int i : data) {
			...
			if(toWriteHigher != 0) compressed[compressedIndexOffset + 1] |= toWriteHigher;
	...
	public static int[] decompress(int bitCount, int[] compressed, int dataLength) {
		...
		for(int i = 0; i < dataLength; i++ ) {
			...
			if(overflow > 0) block |= (compressed[compressedIndexOffset + 1] & ( (1 << overflow) - 1)) << (bitCount - overflow);
			data[i] = block;

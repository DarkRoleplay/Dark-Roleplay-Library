...
import net.dark_roleplay.library.unstable.experimental.blueprints.v2.exception.DataCorruptedException;
...
public final class LoaderBlueprintHeader {
	...
	public static BlueprintHeader loadBlueprintHeader(NBTTagCompound compound) throws UnsupportedVersionException{
		if(compound.hasKey(LoaderV2.KEY_VERSION) && compound.getInt(LoaderV2.KEY_VERSION) == 2) {
	...
	protected static final class LoaderV2{
		...
		public static BlueprintHeader loadBlueprintHeader(NBTTagCompound compound) {
			...
			NBTTagList requiredMods = compound.getList(KEY_MODS, NBT.TAG_STRING);
			for(int i = 0; i < requiredMods.size(); i++) header.addRequiredMod(requiredMods.getString(i));
			...
			NBTTagList credits = compound.getList(KEY_CREDITS, NBT.TAG_STRING);
			for(int i = 0; i < credits.size(); i++) header.addCredits(credits.getString(i));

			NBTTagList layers = compound.getList(KEY_LAYERS, NBT.TAG_STRING);
			for(int i = 0; i < layers.size(); i++) header.addLayerFile(layers.getString(i));

			NBTTagCompound custom = compound.getCompound(KEY_CUSTOM);
			for(String key : custom.keySet()) header.addCustomData(key, custom.getCompound(key));
	...
	protected static final class LoaderV1{
		...
		public static BlueprintHeader loadBlueprintHeader(NBTTagCompound compound) {
			...
			NBTTagList requiredMods = compound.getList(KEY_CREDITS, NBT.TAG_STRING);
			for(int i = 0; i < requiredMods.size(); i++) header.addRequiredMod(requiredMods.getString(i));

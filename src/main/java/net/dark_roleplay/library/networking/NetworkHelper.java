...
public class NetworkHelper {
	...
	/**
	 * Used to set the SimpleChannel, to which {@link SimplePacket} are
	 * registered
	 * when using {@link NetworkHelper#registerPacket(Class)}
	 * 
	 * @param channel
	 */
	...
	/**
	 * Clears your SimpleChannel so other mods don't accidently register their
	 * Packets to your SimpleChannel
	 */
	...
	/**
	 * Warning:
	 * Changing the Order in which your Packets are registered, will also change
	 * their IDs.
	...
	public static <T extends SimplePacket<T>> void registerPacket(Class<T> packetClass) {
		if(CURRENT_CHANNEL == null) {
			...
			CURRENT_CHANNEL.<T>registerMessage(CURRENT_PACKET_ID++ , packetClass, (msg, packetBuffer) -> msg.encode(msg, packetBuffer), (packetBuffer) -> {
				try {
